...
def convert_currency(text: str) -> str:
    ...
    text = re.sub(
        r"(\d+(?:[.,]\d+)*)đ(?![a-zà-ỹ])", replace_vnd, text, flags=re.IGNORECASE
    )
    ...
    text = re.sub(
        r"(\d+(?:[.,]\d+)*)\s*(?:USD|\$)", replace_usd, text, flags=re.IGNORECASE
    )
...
def convert_date(text: str) -> str:
    ...
    text = re.sub(
        ...
        lambda m: f"{m.group(1)} ngày {number_to_words(m.group(2))} tháng {number_to_words(m.group(3))} năm {number_to_words(m.group(4))}",
        text,
    )
