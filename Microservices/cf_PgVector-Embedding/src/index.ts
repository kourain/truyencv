...
export default {
  async fetch(request, env, ctx): Promise<Response> {
    ...
    if (path === "/embed-one" && request.method === "POST") {
      // In a real-world application, you could read content from R2 or
      // a SQL database (like D1) and pass it to Workers AI
      const request_json = await request.json();
      const modelResp: AiTextEmbeddingsOutput = await env.AI.run(
        "@cf/google/embeddinggemma-300m",
        {
          text: [request_json["text"] as string],
        },
      );
    ...
    if (path === "/embed" && request.method === "POST") {
      ...
      const modelResp: AiTextEmbeddingsOutput = await env.AI.run(
        "@cf/google/embeddinggemma-300m",
        {
          text: request_json["texts"] as string[],
        },
      );
    ...
    //   const request_json = await request.json();
    //   let userQuery = request_json["texts"][0];
